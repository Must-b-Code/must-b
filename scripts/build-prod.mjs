...
const BASE_FLAGS = [
  ...
  "--platform=node",
  "--target=node20",
  // No source map in the published bundle — a source map would expose the full
  // first-party backend source (proprietary). Debug locally with a separate build.
  "--external:ethers",
  "--external:@polymarket/clob-client-v2",
  ...
  // bindings resolves module_root via stack trace — breaks when inlined (same class as sharp)
  "--external:@nut-tree-fork/*",
  // inquirer is kept external so Node resolves it from node_modules at runtime
  "--external:inquirer",
  // src/core sub-package dependencies — not in root node_modules, kept external
  "--external:tslog",
  "--external:json5",
...
// No .map copied — source maps are intentionally not produced for the publish build.
...
console.log(sep);
console.log(`\n  ${c.bold("Run (recommended):")}`);
console.log(`    node dist/index.cjs web`);
console.log(`\n  ${c.bold("Run (custom port):")}`);
console.log(`    MUSTB_PORT=4309 node dist/index.cjs web`);
if (BINARY) {
...
console.log(`\n  ${c.dim("Built by Must-b Inc. · https://must-b.com")}\n`);
